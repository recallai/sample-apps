...
import { AudioMixedRawDataEventSchema } from "./schemas/AudioMixedRawDataEventSchema";
 ...
 */
function get_stream_key(args: { recording_id: string }) {
...
function get_audio_passthrough_raw(args: { recording_id: string }) {
    const { recording_id } = args;
    const stream_key = get_stream_key({ recording_id });

    if (!audio_stream_passthroughs_raw.has(stream_key)) {
        const output_path = path.join(
            process.cwd(),
            `output/recording-${recording_id}/mixed.raw`,
        );
...
function get_audio_passthrough_mp3(args: { recording_id: string }) {
    ...
    if (!audio_stream_passthroughs_mp3.has(stream_key)) {
        const output_path = path.join(
            process.cwd(),
            `output/recording-${recording_id}/mixed.mp3`,
        );
        fs.mkdirSync(path.dirname(output_path), { recursive: true });
...
/**
 * Event handler for handling mixed-audio-stream event.
 */
export function bot_mixed_audio_stream_event_handler(args: { msg: Record<string, any> }) {
    const { msg: json_msg } = args;
    const msg = AudioMixedRawDataEventSchema.parse(json_msg);
