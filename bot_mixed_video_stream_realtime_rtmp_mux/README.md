...
This example demonstrates how to live stream the mixed video from a meeting bot to Mux via RTMP. The stream is 720p at 30fps and includes all participants in the bot's current view (speaker or gallery view).

## How It Works

1. User enters a meeting URL in the web client
2. Server creates a Mux live stream and a Recall bot with RTMP streaming
3. Recall streams the mixed video directly to Mux via RTMP
4. The web client displays the live stream
...
4. Watch the live stream in the embedded player

## Stream Specifications

- **Resolution:** 720p at 30 FPS
- **Latency:** Low-latency mode (~5-10 seconds)
