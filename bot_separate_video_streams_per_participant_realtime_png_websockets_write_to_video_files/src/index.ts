...
import { bot_separate_png_video_streams_event_handler, close_png_video_streams_event_handler } from "./bot_separate_png_video_streams_event_handler";
...
wss.on("connection", (socket: WebSocket & { recording_id: string }) => {
    ...
    socket.on("message", (raw_msg) => {
        ...
        // This is needed to close the video pipeline when the socket is closed.
        ...
        // Handle the png video stream event.
        bot_separate_png_video_streams_event_handler({ msg });
    });

    socket.on("close", () => {
        close_png_video_streams_event_handler({ recording_id: socket.recording_id });
